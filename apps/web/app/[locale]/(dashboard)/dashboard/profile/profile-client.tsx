...
import { Loader2, User, GraduationCap, Briefcase, ScrollText, BookOpen, Users, UserCheck, ArrowRight } from 'lucide-react'
...
interface ProfileClientProps {
    initialProfile?: ApiResponse<ApplicantProfileWithRelations>
    initialReferees?: ApiResponse<any[]>
    initialDocuments?: ApiResponse<ApplicantDocument[]>
}

export function ProfileClient({ initialProfile, initialReferees, initialDocuments }: ProfileClientProps) {
    const { data: profileResponse, isLoading } = useMyProfile(initialProfile)
    const { data: refereesResponse } = useMyReferees(initialReferees)
    const { data: documentsResponse } = useMyDocuments(initialDocuments)
